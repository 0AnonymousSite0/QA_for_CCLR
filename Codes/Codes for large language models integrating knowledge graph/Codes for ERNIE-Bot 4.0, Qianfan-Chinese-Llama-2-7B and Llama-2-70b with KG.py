import os
import pandas as pd
os.environ['QIANFAN_AK'] = "8Lsze2UI6kgImgiclqw3jglc"
os.environ['QIANFAN_SK'] = "ZozCpLvQ2kPeiapQfyRwSjAjGVFMND3q"
...
llm_llama_2_70b = QianfanChatEndpoint(streaming=True, model="Llama-2-70b-chat", temperature=0.01,top_p=0,penalty_score=1)
llm_ERNIE_Bot_turbo40 = QianfanChatEndpoint(streaming=True, model="ERNIE-Bot-4",temperature=0.01,top_p=0,penalty_score=1)
llm_Qianfan_Chinese_Llama_2_7B = QianfanChatEndpoint(streaming=True, model="Qianfan-Chinese-Llama-2-7B", temperature=0.01,top_p=0,penalty_score=1)
...
for year in years:
  ...
  df = pd.DataFrame(columns=["Question","Correct_Answer",'Answer1', "Score1","Answer2","Score2","Answer3","Score3","Answer4","Score4","Answer5","Score5","Answer6","Score6","Answer7","Score7","Answer8","Score8","Answer9","Score9"])
  for i in range(len(Questions)):
    ...
    df2 = pd.DataFrame([{"Question":Questions[i],"Correct_Answer":Answers[i],
                         ...
                         "Answer6":result_llm_ERNIE_Bot_turbo40['result'],"Score6":final_score(Questions[i],result_llm_ERNIE_Bot_turbo40['result'],Answers[i]),
                         "Answer9": result_llm_Qianfan_Chinese_Llama_2_7B['result'], "Score9": final_score(Questions[i], result_llm_Qianfan_Chinese_Llama_2_7B['result'],Answers[i])}])
  ...
  save_df_to_excel(df, "Answer_from_First_Level_in_RCQE"+year+".xlsx", "sheet1")
