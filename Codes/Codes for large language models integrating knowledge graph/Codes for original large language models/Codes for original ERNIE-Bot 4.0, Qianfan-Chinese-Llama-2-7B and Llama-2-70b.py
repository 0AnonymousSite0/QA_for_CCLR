...
import pandas as pd
import os
chat_comp = qianfan.ChatCompletion(ak="UPSYGwsZ5pm0Oi0ZUV4dNn8L", sk="KhGSMMiVskYVFOflZQTdbOkcTDOGszzb")
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    n=n+1
    message=Questions[i]
    result_llm_llama_2_70b=chat_comp.do(model="Llama-2-70b-chat", messages=[{"role": "user","content": message}])
    result_llm_ERNIE_Bot_turbo40=chat_comp.do(model="ERNIE-Bot-4", messages=[{"role": "user","content": message}])
    result_llm_Qianfan_Chinese_Llama_2_7B=chat_comp.do(model="Qianfan-Chinese-Llama-2-7B", messages=[{"role": "user","content": message}])
  ...
  save_df_to_excel(df, "Answer_from_original_llama2-70b_L22022" + year + ".xlsx", "sheet1")
