...
# model = AutoModel.from_pretrained("/root/chatglm2-6b/", trust_remote_code=True,top_p=0, 
#   temperature=0).cuda()
tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/ChatGLM3_6b_fine_tunedPM&calculation_e1/", trust_remote_code=True)
model = AutoModel.from_pretrained("/root/autodl-tmp/ChatGLM3_6b_fine_tunedPM&calculation_e1/",  trust_remote_code=True,top_p=0, 
  temperature=0).cuda()
# modelPath = os.path.abspath('../chatglm2-6b')
...
#
    ...
    print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_original_chatglm3-6b_fine_tuned:",answer3.strip().replace("\n",""))


    df = pd.concat([df, df2], axis=0)

  save_df_to_excel(df, "All_answers_from_Original_ChatGLM3_6B"+year+".xlsx", "sheet1")
